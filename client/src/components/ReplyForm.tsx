...
import { replyAction } from "../store/customActions/replyAction";
...
const ReplyForm: FC<{ close: any; tweetId: string }> = ({ close, tweetId }) => {
  ...
  const loading = useSelector((state: RootState) => state.app.reply).loading;

  const onSubmit = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();

    dispatch(replyAction(text, tweetId, source, close));
  };
  useEffect(() => {
    return () => {
  ...
  return (
    <form onSubmit={onSubmit} className="reply-form init-animation">
      ...
      <div className="reply-form__footer">
        ...
        <button
          className={`btn ${loading || !text.trim() ? "disable-btn" : ""}`}
        >
          Reply
        </button>
      </div>
